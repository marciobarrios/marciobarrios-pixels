...
Hover the portrait for one second or focus it for a Konami-code hint. Enter ↑ ↑ ↓ ↓ ← → ← → B A anywhere on the page to open the animated “Beyond the pixels” dialog; on touch screens, tap the portrait and use the on-screen controls. The six personal details are adapted from the section after “Still here? Do you want to know more about me?” on [marciobarrios.com](https://marciobarrios.com/), with playful copy, the original links, and Marcio’s updates about living by the sea in Badalona and his old Made in Honduras side project. The listener ignores text fields, other dialogs, modifier shortcuts, and held-key repeats. Escape, the close button, or the backdrop dismiss the dialog and restore focus; reduced motion is respected.
